...
def load_labels_and_losses(grasp_estimator, contact_infos, global_config, train=True):
    ...
    (
        tf_pos_contact_points,
        tf_pos_contact_dirs,
        tf_pos_contact_approaches,
        tf_pos_finger_diffs,
        tf_scene_idcs,
    ) = load_contact_grasps(contact_infos, global_config["DATA"])

    iterator = None
    idx = scene_idx_pl
    if train:
        grasp_dataset = tf.data.Dataset.from_tensor_slices(
            (
                tf_pos_contact_points,
                tf_pos_contact_dirs,
                tf_pos_contact_approaches,
        ...
        (
            tf_pos_contact_points_idx,
            tf_pos_contact_dirs_idx,
            tf_pos_contact_approaches_idx,
            tf_pos_finger_diffs_idx,
            tf_scene_idcs,
        ) = next_element
        idx = 0
    ...
    (
        ...
        grasp_suc_labels_pc,
        approach_labels_pc,
    ) = grasp_estimator._model_func.compute_labels(
        tf_pos_contact_points_idx[idx],
        tf_pos_contact_dirs_idx[idx],
        tf_pos_contact_approaches_idx[idx],
        tf_pos_finger_diffs_idx[idx],
        target_point_cloud,
        cam_poses_pl,
        global_config,
    )
    ...
    (
        ...
        adds_loss,
        adds_loss_gt2pred,
    ) = grasp_estimator._model_func.get_losses(
        target_point_cloud,
        end_points,
        dir_labels_pc_cam,
        offset_labels_pc,
    ...
    if global_config["MODEL"]["pred_contact_approach"]:
        total_loss += (
            global_config["OPTIMIZER"]["approach_cosine_loss_weight"] * approach_loss
        )
    ...
    if global_config["MODEL"]["pred_grasps_adds_gt2pred"]:
        total_loss += (
            global_config["OPTIMIZER"]["adds_gt2pred_loss_weight"] * adds_loss_gt2pred
        )
    ...
    loss_label_ops = {
        ...
        "offset_label_idcs_pc": tf.argmax(offset_labels_pc, axis=2)
        if global_config["MODEL"]["bin_offsets"]
        else None,
        "offset_orig_labels_vals": orig_offset_labels
        if global_config["MODEL"]["bin_offsets"]
        else None,
        "offset_bin_label_vals": tf.gather_nd(
            tf_bin_vals, tf.expand_dims(tf.argmax(offset_labels_pc, axis=2), axis=2)
        )
        if global_config["MODEL"]["bin_offsets"]
        else None,
...
def load_contact_grasps(contact_list, data_config):
    ...
    for i, c in enumerate(contact_list):
        contact_directions_01 = (
            c["scene_contact_points"][:, 0, :] - c["scene_contact_points"][:, 1, :]
        )
    ...
    device = (
        "/cpu:0"
        if "to_gpu" in data_config["labels"] and not data_config["labels"]["to_gpu"]
        else "/gpu:0"
    )
