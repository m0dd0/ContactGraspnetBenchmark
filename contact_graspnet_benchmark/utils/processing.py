...
from contact_graspnet_benchmark.orig.contact_graspnet.data import (
...
class End2EndProcessor:
    def __init__(self, checkpoint_dir: Path, forward_passes: int = 1):
        self.global_config: Dict[str, Any] = config_utils.load_config(
            checkpoint_dir, batch_size=forward_passes
        )
        ...
        self.grasp_estimator.load_weights(
            self.sess, self.saver, self.checkpoint_dir, mode="test"
        )

    def __call__(self, sample_path):
